...
("Presented at CODE/GEO/GRAPHIC, Berlin, 19 April 2018")
...
library(maptools)
library(sp)
# library(rnaturalearth) # <- source for geodata, but pkg masks some needed f()
```


### 2.2 Geodata: World Map Data from Natural Earth
*"Natural Earth is a public domain map dataset available at 1:10m, 1:50m, and 1:110 million scales. Featuring tightly integrated vector and raster data, with Natural Earth you can make a variety of visually pleasing, well-crafted maps with cartography or GIS software."* [Natural Earth](http://www.naturalearthdata.com/)
...
**Spoiler: Some Objects – which were only mentioned later on – are missing at Admin-0 level.**
...
(this was done before [geom_sf / ggsf](http://ggplot2.tidyverse.org/reference/ggsf.html) where introduced to [ggplot2](http://ggplot2.tidyverse.org/index.html). There might exist a 
more elegant solution right now, esp. with ggplot.)
...
#### Raster for the rest (oceans & terrain)
```{r}
...
plot(relief.meer)
# reliefplot2 <- crop(reliefplot2, box)
# plot(reliefplot2)
```
