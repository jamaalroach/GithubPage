...
Anyways, a dataset of this size is a perfect exerciser in data wrangling and exploratory analysis with tools from the galactic `tidyverse`. So what I'm aiming to highlight with this post, is my more or less systematic approach to turning an granular dataset into something more useful (and reliable!) for further in-depth analysis.
...
# save(data_raw, file = str_c(data_path, "infoops_data.RData"))
...
We can already make some interesting observations from this summary alone:
  ...
  * only ~~50K~~ 266.208 Tweets are replies -> rather few interactions
  ...
  * all the `$*_tweetid` were read as numeric, which we'll also need to change, as IDs are supposed to be unique identifiers and not continuous values -> `mutate()`
  ...
  * there are 608 unique account locations (from an unknown number of accounts at this point), and there are 4.779 geolocated Tweets. That's not much, but we could try to double-check these locations with the respective `$account_language` values.
    
That should be enough leads for an initial inquiry. Let's continue with the data preparation and address what we have discovered so far.
...
We need to reduce the scope of Tweet languages for now. 5000 is only slightly less than 1% of the unique Tweets, so this sound like a good threshold.
...
We're down to `r n_distinct(data_unique$tweet_language)` language groups for the Tweets. That's far better!
...
For `$Tweet_Langs_Counts`, we have utilized the quite elegant `forcats::fct_count()` which gives us a `list` of of the grouped language counts.
...
From what we can see here, is that we have quite an amount of "influencers" - accounts with lots of followers and low rates of following others.
...
We can see from both plots that the most influential or active accounts were created in 2014 or later, and that the relation between Russian and English-language account languages is rather balanced in terms of max. Tweet numbers. However, English-speaking accounts are more dominant in terms of numeric dominance (Following/Followers).
...
Now let's visualize all the 57 Account Language -> Tweet Language combinations.
