...
Notice that you can achieve 0-18 points, where 8/9 allows to become a judge and 10 means outstanding...

### Import
Let's fetch the date from the official page of the Berlin Senate. You'll get a PDF which you have to destill with the tabulizer package (or by hand) in order to get a CSV.
...
### Skim / Preview
The skimr-Package is great for quickly inspecting the kind of data you get.
```{r}
skimr::skim(noten_raw)
```

(In RStudio / R Markdown the hist column is rendered properly. You get a nice histogram per (numeric) variable:)
![(Screenshot of skimr from RStudio)](/img/Staatsexamen_Skimr.jpg)
