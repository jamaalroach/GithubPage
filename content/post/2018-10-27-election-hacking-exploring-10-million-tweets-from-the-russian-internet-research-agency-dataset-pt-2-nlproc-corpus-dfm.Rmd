---
title: 'Election Hacking: Exploring 10 Million Tweets from the Russian Internet Research
...
description: "This is the sceond part of my \"Internet Researc Agency\" series. We'll encounter in-memory computing problems, adress them with tweaks on Windows and Linux, and finally succeed at building a 5.7M x 1.5M big sparse Document-Feature Matrix."
abstract: "This is the sceond part of my \"Internet Researc Agency\" series. We'll encounter in-memory computing problems, adress them with tweaks on Windows and Linux, and finally succeed at building a 5.7M x 1.5M big sparse Document-Feature Matrix."
...
sequenceDiagrams:
  ...
  options: ''
---
![*(73 Topics from 2M English-language Tweets; modelled with the stm()-packages; K auto-induced with t-SNE/PCA)*](/img/IRA_dataset/IRA_keywords.png)

Now that we are done with [pre-processing the Internet Research Agency dataset](election-hacking-exploring-2-million-tweets-from-the-russian-internet-research-agency-dataset-pt-1), we can unleash the full R powers and dive into automated content analysis. For a primer, see the following excellent intros and posts re: core NLP concepts, R packages, and tools:
...
**Bonus**: [Carsten Schwemmer](https://twitter.com/c_schwemmer){target="_blank"} (who introduced me to the `Tidyverse` and automated content analysis with R, btw!) has build an `Shiny` app, which allows you to explore Structural Topic Models with a browser-based GUI: [stminsights - A Shiny Application for Structural Topic Models](https://cschwem2er.github.io/stminsights/){target="_blank"}

> Just a word of caution: Topic Modelling is a statistical method, and doing it the way I'm doing here - on the full 5.7M Tweets corpus OR even on a subset of 2M Tweets without any frequency-based feature reduction (i.e. with `quanteda::dfm_trim()`) - will take some time. A lot of time actually. I have a decent Gamer's Laptop with 16GB RAM, a quad-core Intel i7-7500 CPU and a SSD (SATA tho) - and computing the Topic model on the English subset  (2M Tweets, with auto-induced K=73 topics, based on 15K features -> 3rd Post) took a whole day (22,5 hours). Don't try this at home or without some free AWS credits :)

# Packages & Data

Let's get started. Here are some tweaks/tricks/adjustments I discovering along writing this ([in this 8 years old Stack Overflow post, for instance](https://stackoverflow.com/questions/5171593/r-memory-management-cannot-allocate-vector-of-size-n-mb){target="_blank"}) in order to increase the computing performance ;)
...
> Save any large output as an `.rds` or `.RData` (if you can afford) to your harddrive, clear your environment and start the next step with as low of a RAM footprint as possible.
...
Since we've already [pre-processed the Internet Research Agency dataset](election-hacking-exploring-2-million-tweets-from-the-russian-internet-research-agency-dataset-pt-1), we can just load the exported data object and go on with corpus-building.
...
Let's see what we can get observe by plotting the **Tweet Languages** and inspect the top peaks' dates. To further reduce noise, we'll filter out languages with less than 500 Tweets per week from 2012 on (as we have aggregated the Tweets on a weekly basis, in fact, only week-wise values will be filtered).
...
Those patterns are really telling! And we can add one more helper: Text labels (the Mondays for the respective weeks) for those top peaks (global maxima), colored by the respective Account languages: 
...
> Technically, we could repeat this for all peaks of any particular Tweet or Account language and find out more about the particular events (.i.e. German-language Tweets peaking around the Federal Elections in autum 2017 - barely visibly with this color palette) with the help of Wikipedia's super-handsome [current events portal](https://en.wikipedia.org/wiki/Portal:Current_events#Events_by_month){target="_blank"}.

For instance, we can observe **Russian-language** Tweets starting to peak in the beginning of **2014**:
...
As for the **English-language** Tweets (attributed to the Russian "Internet Research Agency" in this dataset), we see them gain momentum starting in **mid 2014**, shortly after the shot-down of MH17... 
...
* (and Neymar was bought by PSG for €222 million on the same day, which might explain why Spanish Accounts where most active)
...
Just for comparison (of plain numbers to the usefulness of exploration by data visualization), here's some tables of data we just visualized. You can easily identify the dates of the events from the numbers alone. But in order to recognize patterns, esp. language patterns, a visual representation is really helpful. However, without this table I probably would have missed the **Spanish**-language Accounts peaking with Tweets in English in two subsequent weeks...
...
> Now let's try to do something really **naive**: We'll build a corpus from **all Tweets** - with **all languages**.\nThis will take a minute ot two (actually: each step took me **hours**...) with `corpus()`, and `dfm()`, but this way we can explore a lot of presumed topics in various languages early on.\nAdd **test** where the limits of in-memory computing with text as data might be with R ("Big Data calling")...
...
* save the data object to the hard-drive
* clear your R environment with rm() and gc() (<- garbage collection)
* restart R
* load the data object from the harddrive
...
## Corpus Exploration: Key-Word-in Context
...
We'll put the keywords into a (`kwic()`-friendly) vector and then do a single run with all keywords instead of repeating this n-times. This will take time and might not work properlly without enough RAM.
...
kwic_results %>% 
    ...
    select(pre, keyword, post, tweet_time, account_language, tweet_language) %>%
  head(20) %>% 
  knitr::kable("html")
```
...
After failing to build a `dfm` from the untouched `corpus` repeatedly (on Windows & Linux), this SO-post explained how to reduce the number of features in the corpus by tokenizing the texts and removing unwanted tokens **before** creating the `dfm`. This way I really gained some momentum for `dfm()` (by reducing dimensions), but we would also loose the analytical methods / functionality of a corpus object (i.e. `kwic()`) as we destroy the basic corpus structure (full Tweets in our case) by replacing the texts with tokens. This experiment was more of a permormance evaluation than a recommended practice (for now, at least). I ended up with something like a 60M x 1.5M DFM, which obviously shows that I had lost the document-based corpus structure.

Having said that, here's our **regular strategy**:

* create untouched corpus (we did this above, already)
* ~~remove obsolete tokens from corpus by directly tokensing the texts~~
...
First of all, we will need to create list of very frequent tokens/words which we can simply filter out from our corpus. This is my current **best-practice** with regards to stopwords. I'm using three different sources to compile a super-list of stopwords for the language concerned. (As for "amp", this is an artefact I learned from my [previous posts](https://ellocke.github.io/post/r-german-academic-twitter-pt-2-from-data-to-corpus-with-a-turkish-twist/#inspect-keyword-in-context-kwic-for-amp): It's just a left-over of `&amp`, which is the HTML entitiy for string-parsing `&`, and is created when we use `remove_punct = TRUE`. Same is true for `&quot` and probably some other unparsed/escaped entities in the IRA datsset).
...
> Caution: This is the **taboo**-approach, which I tried out and would not recommend re-doing. It performs. but the analytical value of the output is very limited!

> This task alone would first approach taking up more than 20GB "RAM"" on my Windows OS Laptop and evetually crash, so be warned again: proceed with caution!

> On Ubuntu Linux we'll get along with 9GB-11GB for the same task (corpus building, tokenizing and reducing) without even touching the swap file. This difference is insane... However, this still would not work out for creating a `dfm` from the untouched, all-tweets corpus. If you want to replicate this, I suggest at least 16GB RAM + 20GB swap memory or 32GB RAM and a regular-sized swap file/drive. SSD performance might be better. Or cloud if you're willing to pay 2-3€/hour for the larger instances...
...
We already have a plain, untouched corpus object from further above:
...
We'll load the clean, untouched corpus from our HDD and proceed with `dfm()`
...
Now we'll turn the interational corpus into a sparse [Document-Feature Matrix / Document-Term-Matrix](https://en.wikipedia.org/wiki/Document-term_matrix){target="_blank"} with `quanteda::dfm()` ~~and include n-grams of 1 and 2 words (bigrams)~~.
...
* setting up a USB 3.0 thumb drive as a 64GB swap drive (my current SSD is too small for Windows, Ubununtu + swap partition)
...
Now we'll make sure that there are no left-over 1- and 2-character tokens (which are called features now) and then save the dfm to our HDD.
...
> Holy sh\*t. That's the power of `Quanteda` on Linux I guess. So my "little" proof-of-concept experiment worked out. You **can** build a large dfm from a large corpus without resorting to cloud computing / AWS et al.

Now we can explore a bit more at then proceed over to building a topic model.
...
> Funny! **Russian terms** dominate the international corpus aka Twitter's "Internet Research Agency" dataset :)\nWhat's also noteworthy, is the top-ranked \#news hashtag, probably pointing to the proliferation of the "fake news" term.

Now let's create a subset of our dfm without \#hashtags and @account handles for a purer content analysis. It's a bit redundant, but due to the (in-memory) size of the full-languages dfm (2.8GB without hashtags/accounts; 2.7GB w/o) this is neccessary, if want to work with and without Twitter artefacts.
...
> We've further reduced the number of features from 1.530.058 to 965.278 by removing Twitter Account @handles and \#hashtags
...
![*(73 Topics from 2M English-language Tweets; modelled with the stm()-packages; K auto-induced with t-SNE/PCA)*](/img/IRA_dataset/topic_modelling.jpg)
