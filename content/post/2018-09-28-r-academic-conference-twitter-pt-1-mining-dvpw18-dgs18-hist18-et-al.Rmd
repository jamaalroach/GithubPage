---
title: '[R] Academic Twitter, Pt. 1: Mining #dvpw18, #dgs18, #hist18 et
  al.'
author: Ilja / fubits
date: '2018-09-28'
categories:
...
description: In September, four big academic societies in Germany had their annual meeting - at the same time! You can **not not** harvest their tweets
...
sequenceDiagrams:
  ...
  options: ''
---
![](/img/Twitter_conf/Twitter_conf.png "This is what 4 German academic conferences look like on Twitter"){width=100%}

As (bad) luck has it, four big academic societies in Germany somehow decided to hold their respective annual meetings within the same week:

  * [Political Science](https://www.dvpw.de/kongresse/dvpw-kongresse/dvpw2018/){target="_blank"} (#dvpw18, #dvpw2018, #dvpw)
    
  * [Sociology](https://kongress2018.soziologie.de/aktuelles/){target="_blank"} (#dgs18, #dgs2018)
    
  * [History](https://www.historikertag.de/Muenster2018/){target="_blank"} (#histag18 / #histag2018 / #historikertag2018)
    
  * [Computer Science](https://informatik2018.gi.de/){target="_blank"} (informatik2018)
        
        
Even though Germany is still a bit behind with regards to Twitter, four conferences = 4x the chance to work on your Twitter mining and text wrangling skills ;). Plus, we get some interesting data for the future practice of our NLP / text processing and social network analysis skills...
...
ggplot(dvpw_counts, aes(x = Favs, y = RT)) +
  ...
  labs(size = "Anzahl Tweets",
       ...
       subtitle = paste0("(n = ", dvpw_n_tweets,
                       " Tweets von m = ", dvpw_n_accounts,
                       " Accounts; Stand: 29.09.18, ", timeString , " Uhr;"),
       x = "Anzahl Favourites",
       y = "Anzahl Retweets",
       caption = "@fubits")
```
...
Let's have look, how German Sociologists performed on Twitter. Like above, I've mined the Tweets multiple times in order to get a good sample.
...
dgs_collection %>%
  ...
  guides(fill = FALSE)
```
That looks rather different from the #dvpw community. Less institutional dominance and actually, much more active Twitter users (697 active users vs 238 in team PolSci).
...
Now, let's look at German History Scholars.
...
all_n_tweets
```
So, this week, ```all_n_accounts``` German academic Twitter accounts have been active at four conferences in total, producing ```all_n_tweets``` individual Tweets. Actually, that's quite impressive!
...
all_cons_per_user %>% head(10)
```

## Joint Scatterplot: per-User
Let's have a look at the German academic Twitter crowd:
```{r fig.width=12}
ggplot(all_cons_per_user, aes(x = Favs, y = RT, color = discipline)) +
    ...
    labs(size = "Anzahl Tweets",
         color = "Disziplin",
         title = "Twitter-Aktivität zu Twitter-Aktivität zu #dvpw*18, #dgs*18, #hist*18, #informatik2018: Top 20 Accounts",
