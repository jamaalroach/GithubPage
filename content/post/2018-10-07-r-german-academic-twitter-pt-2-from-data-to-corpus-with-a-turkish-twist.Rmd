...
Please see [Twitter pt. 1](https://ellocke.github.io/post/r-academic-conference-twitter-pt-1-mining-dvpw18-dgs18-hist18-et-al/) for Twitter Mining with `rtweet` and the details of the data handling approach.
...
mixed_collection %>% count() # %>% knitr::kable()
```
...
```{r echo=TRUE}
mixed_collection %>% arrange(created_at) %>% select(text) %>% head(10) # %>%
...
```{r}
topfeatures(joint_dfm, 20, groups = "Discipline") # %>% map(unlist) %>% map(knitr::kable)
```
